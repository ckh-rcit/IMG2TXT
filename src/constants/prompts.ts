...
const FLUX_GROUNDING_RULES = ' Anchor strictly to what is directly visible in the input image. Do not infer unseen scenes or reinterpret the image as a collage, panel grid, infographic, poster, flat lay, or studio product layout unless those elements are clearly visible. Start with a literal scene anchor naming the main subject and action.'
...
export function buildPrompt(type: PromptType | 'ocr', detail: string, sceneAnchor?: string): string {
  const base = BASE_PROMPTS[type]
  if (type === 'ocr') return base
  const anchorRule = sceneAnchor
    ? ` Begin the prompt with this scene anchor and keep all details consistent with it: ${sceneAnchor}.`
    : ''
  return base + DETAIL_MODIFIERS[detail as DetailLevel] + anchorRule
}
